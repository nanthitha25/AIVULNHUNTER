...
export default function AiAssistant() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { role: 'assistant', content: 'Hi! I am the AIVulnHunter Security Assistant. I can help answer your security questions. Ask me things like "What is SQL Injection?" or "How to mitigate XSS?"' }
    ]);
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50">
            {/* Chat Window */}
            {isOpen && (
                <div className="mb-4 w-80 sm:w-96 h-[550px] max-h-[80vh] bg-[#0a0514] border border-purple-500/30 rounded-2xl shadow-2xl shadow-purple-900/50 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5">
                    {/* Header */}
                    <div className="bg-gradient-to-r from-purple-900 to-indigo-900 p-4 flex justify-between items-center border-b border-purple-500/20">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-white font-bold text-sm tracking-wide uppercase">Security Assistant</h3>
                        </div>
                        <button 
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-purple-900 scrollbar-track-transparent">
                        {messages.map((msg, idx) => (
                            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[90%] rounded-xl p-3 text-sm shadow-md ${
                                    msg.role === 'user' 
                                        ? 'bg-purple-600 text-white rounded-tr-none shadow-purple-900/40' 
                                        : 'bg-gray-800/80 text-gray-200 border border-gray-700 rounded-tl-none shadow-black/40'
                                }`}>
                                    {msg.role === 'assistant' ? (
                                        <div className="prose prose-invert prose-sm max-w-none prose-headings:text-purple-400 prose-headings:font-bold prose-headings:mb-2 prose-headings:mt-4 first:prose-headings:mt-0">
            ...
            {!isOpen && (
                ...
                >
                    ...
                    <span className="font-bold text-sm tracking-wide">Ask Security AI</span>
                </button>
            )}
        </div>
    );
